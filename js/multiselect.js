!function ($) {
	...
	Multiselect.prototype = {
		...
		createOptionValue: function(element, wrapper) {
			if ($(element).is(':selected')) {
				$(element).attr('selected', 'selected').prop('selected', true);
			}
			wrapper = wrapper || '.multiselect-container';
			...
			$(wrapper, this.$container).append($li);
		...
		buildDropdown: function () {
			...
			this.$select.children().each($.proxy(function (num, element) 
			{
				...
				if (tag == 'optgroup') {
					...
					var $li = $('<li class="group-label"><label style="margin:0;padding:3px 20px 3px 20px;height:100%;"></label></li>');
					var $ul = $('<ul class="multiselect-group"></ul>');
					$('label', $li).text(groupName);
					$ul.attr('id', 'optgroup-'+num);
					$li.append($ul);
					$('.multiselect-container', this.$container).append($li);
					
					// Add the options of the group.
					$('option', group).each($.proxy(function (index, element) {
						this.createOptionValue(element, '#optgroup-'+num);
					}, this));
				}
				else if (tag == 'option') {
